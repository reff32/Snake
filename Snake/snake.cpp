...
Snake::Snake(int l_blockSize, Textbox* l_textbox)
{
	m_textbox = l_textbox;
	m_size = l_blockSize;
	//m_bodyRect.setSize(sf::Vector2f(static_cast<float>(m_size - 1), static_cast<float>(m_size - 1)));// размер ячейки на один пикс меньше чем m_size
...
/*Direction Snake::GetPhysicalDirection()
{
	...
	SnakeCell& head = m_snakeBody[0];
	SnakeSegme& neck = m_snakeBody[1];
